...
原文请参考TP3的开发手册: [ThinkPHP3.2.3完全开发手册-配置-配置加载](https://www.kancloud.cn/manual/thinkphp/1690)
...
也就是说, 当配置内容发生冲突时, 动态配置的优先级最高, 扩展配置的优先级其次, 模块配置的优先级第三, ..., 惯例配置的优先级最低.

这个很好理解. 就和CSS渲染HTML一样.

惯例配置文件位置: TP3项目根目录/ThinkPHP/Conf/convention.php<br/>
应用配置文件位置: TP3项目根目录/Application/Common/Conf/config.php<br/>
调试配置文件位置: <br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;框架的调试配置文件: TP3项目根目录/ThinkPHP/debug.php<br/>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;应用的(开启调试模式时)调试配置文件: TP3项目根目录/Application/Common/Conf/debug.php<br/>
模块配置文件位置: TP3项目根目录/Application/当前模块名/Conf/config.php 


TP3框架给的惯例配置中有以下这些内容, 需要注意了解一下. 不过这个`=>`号的空格方式不对, 应该是`=>`号左右两边各空一个格. 这个在PSR上有例子; 再就你每个半角逗号如果没什么大问题的话都应该空一个空格, 又不是写前端. 还有日志级别原文写的是`EMERG,ALERT,CRIT,ERR`, 如果是我的话我个人会设成`EMERGENCY, ALERT, CRITICAL, ERROR`. 这样显得比较美观和专业, 语义性也有所提升.
